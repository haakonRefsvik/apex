...
### 3.1.3 Room database
The Room database serves as a timesaver in our app, and takes away a lot of the workload the app needs to uphold. It adds features to the app such as saving locations picked from the Mapbox-map, so that the user can have them stored instead of having to remember specifically where the desired point was. By saving data, such as favorite location and favorite weathercard based on timestamp, locally on the running device, the app holds on to those values even after closing the app. 

...
fun simulateTrajectory(
    ...
    while (z >= altitude){
        ...
        if(burnTimeLeft >= 0 && z <= apogee){
            ...
            ax = getNearestLevelData(altitudeMeters = z, allLevels = allLevels)?.vComponentValue ?: 0.0
            ay = getNearestLevelData(altitudeMeters = z, allLevels = allLevels)?.uComponentValue ?: 0.0
            az = -g
